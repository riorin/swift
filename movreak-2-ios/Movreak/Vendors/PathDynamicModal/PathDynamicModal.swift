...
class PathDynamicModal: NSObject, UIGestureRecognizerDelegate {
    ...
    func show(modalView view: UIView, inView: UIView) {
        ...
        self.backgroundView.frame = inView.bounds
    ...
    private func setup() {
        ...
        self.tap = UITapGestureRecognizer(target: self, action: #selector(PathDynamicModal.handleBackgroundView(sender:)))
        self.tap.delegate = self
        
        self.pan = UIPanGestureRecognizer(target: self, action: #selector(PathDynamicModal.panBackgroundView(sender:)))
    ...
    private func fadeBackgroundView(fromAlpha: CGFloat, toAlpha: CGFloat, completion: ((Bool) -> Void)?) {
